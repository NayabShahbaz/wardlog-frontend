...
interface DataTableProps {
  columns: Column[];
  data: DataRow[];
  keyField?: string; // Made optional to default to Member 2's backend _id[cite: 20]
}

const DataTable: React.FC<DataTableProps> = ({ columns, data, keyField = "_id" }) => {
  // Helper to handle nested object properties (e.g., "author.name")[cite: 16, 19]
    ...
    <div className="overflow-hidden">
      ...
      >
        <table className="w-full">
          ...
          <tbody>
            {data.map((row, i) => {
              // Ensure unique key using backend _id or fallback index[cite: 20]
              const rowId = row[keyField] || row._id || i;
              return (
                <tr
                  key={`${rowId}-${i}`}
                ...
                >
                  {columns.map((col) => (
                    ...
                    >
                      {col.render ? col.render(row) : toCellContent(getValue(row, col.key))}
                    </td>
                  ))}
                </tr>
              );
                ...
                <div className="flex-1 min-w-0">
                  {nameCol.render ? (
                    ...
                    <p className="text-sm font-semibold text-gray-900 truncate">
                      {toCellContent(getValue(row, nameCol.key))}
                    </p>
                  )}
                </div>
                ...
                {detailCols.map((col) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-800 font-medium text-right max-w-[60%]">
                      {col.render
                        ? col.render(row)
                        : toCellContent(getValue(row, col.key)) || "—"}
                    </div>
                  </div>
                ))}
              </div>
